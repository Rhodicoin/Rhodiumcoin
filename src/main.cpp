...
#if defined(NDEBUG)
# error "Rhodium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x682b1ef39b31ac5d39d130473d953553c4785ab29a96464dc9be857982e215dd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rhodium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rhodium Signed Message:\n";
...
{
    // Rhodium: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rhodium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rhodium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1470476406, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "1470474537";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04999c72ec6d2bce29675b1e79609f99adcb247e91a3656056b590b21352f6c03342b6a00da39ab08ed8eca97a1bf342967012360306bf2dd1606bd62957dd4ae9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1470476406;
        ...
        assert(block.hashMerkleRoot == uint256("0x4fc020d3d6a2c94736b085fbaffc49480162f338a8d6ea171fc13fc4a9568500"));
...
unsigned char pchMessageStart[4] = { 0x8c, 0x1f, 0xe1, 0x50 }; // Rhodium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RhodiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RhodiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RhodiumMiner(CWallet *pwallet)
{
    printf("RhodiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rhodium-miner");
    ...
    try { loop {
        ...
        printf("Running RhodiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RhodiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RhodiumMiner, pwallet));
}
